...
3. Implement your aspect as usual. In any aspect method, to get the <xref:Metalama.Framework.Code.IAttribute>, use the <xref:Metalama.Framework.Code.RefExtensions.GetTarget*?text=IRef.GetTarget()> method. Then use the <xref:Metalama.Framework.Code.IAttribute.ConstructorArguments> and <xref:Metalama.Framework.Code.IAttribute.NamedArguments> properties to get access to attribute data.
